...
class Network(nn.Module):
  ...
  def forward(self, input):
    ...
    for i, cell in enumerate(self.cells):
      if cell.reduction:
        weights = self.SF(self.alphas_reduce)
        n = 3
        start = 2
        ...
        weights = self.SF(self.alphas_normal)
        n = 3
        start = 2
  ...
  def _initialize_alphas(self):
    ...
    for i in range(k):
        self._arch_parameters.append(nn.Parameter(1e-3*torch.randn(1,num_ops),requires_grad=True))#初始化alpha_normal结构系数
    for i in range(k):
        self._arch_parameters.append(nn.Parameter(1e-3*torch.randn(1,num_ops),requires_grad=True))#初始化alpha_reduce结构系数 
    
    self.alphas_normal= self._arch_parameters[:14]
    self.alphas_reduce = self._arch_parameters[14:28]
  ...
  def SF(self,alpha):
    Alpha = torch.ones(14,7)
    softmax = nn.Softmax(dim = 1)
    for i in range(14):
      Alpha[i] = softmax(alpha[i])  
  ...
  def genotype(self):
    ...
    gene_normal = _parse(self.SF(self.alphas_normal).data.cpu().numpy(),weightsn2.data.cpu().numpy())
    gene_reduce = _parse(self.SF(self.alphas_reduce).data.cpu().numpy(),weightsr2.data.cpu().numpy())
